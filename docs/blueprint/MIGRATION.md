...
> 本檔僅記錄「盤點 + 對照表」——**尚未執行任何刪除**。第 3 步 fresh subagent 驗證 PASS 前，
> 一個檔案都不准 `git rm`。零刪除是本次 P2 seeding 任務的明確範圍邊界（Task B 指示）。

---

## 1. `docs/superpowers/specs/SPEC-voice-conversion.md` → 目標 BP 條目
...
**建議：驗證 PASS 後 `git rm docs/superpowers/specs/SPEC-voice-conversion.md`**
（migrated, removable）。
...
- **建議 `git rm`（migrated, removable after verification）**：
...
**下一步（本次任務範圍外）**：待使用者指派 fresh subagent，逐節核對本對照表與 12 個 BP
條目內容 100% 一致、無遺漏，PASS 後才可執行 `git rm docs/superpowers/specs/SPEC-voice-conversion.md`。
